...
def chunk_passages(pages_text, max_chars=2800):
    passages = []
    buffer = ''
    for p in pages_text:
        ...
        for part in parts:
            ...
            if len(buffer)+len(part)+2 > max_chars:
                if buffer: passages.append(buffer.strip())
                buffer = part
            else:
...
LLM_PROMPT = '''
তুমি একজন অভিজ্ঞ প্রশ্ন প্রস্তুতকারী, Bangladesh admission test (Medical/Engineering/Varsity) স্টাইলে প্রশ্ন তৈরি করো।
Generate up to {max_q} MCQs from the passage below. Prefer Bengali if passage is Bengali. Use only facts present in passage.
Respond with a JSON array only. Each item:
{{
  "question": "...",
  "options": {{"A":"...","B":"...","C":"...","D":"..."}},
  "correct_option": "A|B|C|D|A,C|B,D",
  "explanation": "short explanation (Bangla or English)",
  "subject": "Biology|Chemistry|Physics|Math|English|General",
  "chapter": "...",
...
{passage}
---
Exam style: {exam_type}
Subject hint: {subject_hint}
'''

def generate_with_gemini(prompt_text, model_name='gemini-1.5'):
    if not GEMINI_AVAILABLE:
        raise RuntimeError('Gemini not configured')
    resp = genai.generate_text(model=model_name, prompt=prompt_text)
    return resp['candidates'][0]['content']

def generate_mcqs_from_passage(passage, max_q=4, exam_type='General', subject_hint=''):
    prompt = LLM_PROMPT.format(max_q=max_q, passage=passage[:15000], exam_type=exam_type, subject_hint=subject_hint)
    ...
    for item in data:
        ...
        for k in ['A','B','C','D']: opts.setdefault(k,'')
        out.append({
            ...
            'subject': item.get('subject','').strip() or subject_hint,
            'chapter': item.get('chapter','').strip(),
            'difficulty': item.get('difficulty','').strip() or 'Medium',
            'year': item.get('year','').strip() or ''
        })
    return out

def save_csv(questions, path):
    headers = ['subject','chapter','question','option_a','option_b','option_c','option_d','correct_option','explanation','difficulty','year']
    with open(path, 'w', newline='', encoding='utf-8') as f:
        w = csv.writer(f)
        w.writerow(headers)
        for q in questions:
            w.writerow([q['subject'], q['chapter'], q['question'], q['options']['A'], q['options']['B'], q['options']['C'], q['options']['D'], q['correct_option'], q['explanation'], q['difficulty'], q['year']])

def insert_into_db(questions):
    ...
    try:
        with conn.cursor() as cur:
            ...
            def get_or_create_chapter(name, sid):
                if not name or not sid: return None
                cur.execute('SELECT id FROM chapters WHERE name=%s AND subject_id=%s', (name, sid))
                r = cur.fetchone()
                if r: return r[0]
                cur.execute('INSERT INTO chapters (subject_id,name) VALUES (%s,%s)', (sid,name))
                return cur.lastrowid
            for q in questions:
                sid = get_or_create_subject(q['subject'])
                cid = get_or_create_chapter(q['chapter'], sid)
                cur.execute('''INSERT INTO questions (subject_id, chapter_id, question_text, option_a, option_b, option_c, option_d, correct_option, explanation, difficulty, year) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)''',
                            (sid, cid, q['question'], q['options']['A'], q['options']['B'], q['options']['C'], q['options']['D'], q['correct_option'], q['explanation'], q['difficulty'], q['year']))
        conn.commit()
    finally:
        conn.close()
...
PREVIEW_HTML = '''<html><head><meta charset='utf-8'><title>Preview</title></head><body><h2>Preview {{count}} questions</h2><a href='/download/{{csv_name}}'>Download CSV</a><br><a href='/'>Upload another</a>{% for q in questions %}<div style='border:1px solid #ccc;padding:10px;margin:5px;'><b>Subject:</b>{{q.subject}} <b>Chapter:</b>{{q.chapter}}<br><b>Q:</b>{{q.question}}<br>A: {{q.options.A}}<br>B: {{q.options.B}}<br>C: {{q.options.C}}<br>D: {{q.options.D}}<br><b>Answer:</b> {{q.correct_option}} <b>Explanation:</b>{{q.explanation}}</div>{% endfor %}</body></html>'''
